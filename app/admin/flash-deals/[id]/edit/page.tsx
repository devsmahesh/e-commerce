...
import { useUpdateFlashDealMutation, useGetFlashDealByIdQuery } from '@/store/api/flashDealsApi'
...
export default function EditFlashDealPage() {
  ...
  const { data: deal, isLoading: isLoadingDeal } = useGetFlashDealByIdQuery(dealId)
  ...
  const initialValues = deal
    ? {
        title: deal.title,
        description: deal.description || '',
        type: deal.type,
        discountPercentage: deal.discountPercentage?.toString() || '',
        minPurchaseAmount: deal.minPurchaseAmount?.toString() || '',
        link: deal.link || '',
        buttonText: deal.buttonText || '',
        buttonVariant: deal.buttonVariant || 'default',
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    try {
      ...
      toast({
        title: 'Error',
        description: error?.data?.message || 'Failed to update flash deal',
        variant: 'destructive',
      })
    }
  }
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <h1 className="text-4xl font-bold text-slate-900">Flash Deal Not Found</h1>
        </div>
        <Card>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-4xl font-bold text-slate-900">Edit Flash Deal</h1>
          <p className="mt-2 text-slate-600">Update flash deal details</p>
        </div>
      </div>
          ...
          <Form>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        // Set default link and button text based on type if not already set
                        if (!values.link) {
            ...
            <div className="flex justify-end gap-4 pt-6">
              ...
              <Button type="submit" disabled={isLoading || isSubmitting}>
                {isLoading || isSubmitting ? 'Updating...' : 'Update Flash Deal'}
              </Button>
            </div>
          </Form>
        )}
